...
function alura_intercambioa_adicionando_recursos_ao_tema() {
    add_theme_support('custom-logo');
    add_theme_support('post-thumbnails');
}
add_action('after_setup_theme', alura_intercambioa_adicionando_recursos_ao_tema());

$alura_intercambios_registrando_menu = fn() => register_nav_menu(
    'menu-navegacao',
    'Menu navegação'
);
...
function alura_intercambios_registrango_post_customizado_banner(){
    register_post_type(
        'banners',
        array(
...
add_action('init', alura_intercambios_registrango_post_customizado_banner());
...
function ai_funcao_callback($post) {
    ...
    $text_home_1 = get_post_meta($post -> ID, '_texto_home_1', true);
    $text_home_2 = get_post_meta($post -> ID, '_texto_home_2', true);

    ?>
    <label for="texto_home_1">Texto 1</label>
    <input type="text" name="texto_home_1" style="width: 100%" value="<?= $text_home_1 ?>"/>
    <br>
    <br>

    <label for="texto_home_2">Texto 2</label>
    <input type="text" name="texto_home_2" style="width: 100%" value="<?= $text_home_2 ?>"/>
    <?php
}
